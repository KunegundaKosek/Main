...
public class PalindromeDetector {
    public static void main(String[] args) {
        ...
        boolean result = checkIfStringIsPalidromeRecursively(input);
        System.out.println(result);
    }

    private static boolean checkIfStringIsPalidromeRecursively(String input) {
        if (input.length() <= 1) {
            return true;
        }
        if (input.charAt(0) == input.charAt(input.length() - 1)) {
            ...
            return checkIfStringIsPalidromeRecursively(inputWithoutFirstAndLastChar);
        } else {
            return false;
        }
    }
